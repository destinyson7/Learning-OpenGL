...
int main()
{
    ...
    float vertices[N_VERTICES * 8];

    for (int i = 0; i < N_VERTICES * 8; i++)
    {
        in >> vertices[i];
    }
    ...
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Assignment 0 Bonus", NULL, NULL);
    if (window == NULL)
    {
    ...
    unsigned char *data = stbi_load("../source/textures/texture.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
